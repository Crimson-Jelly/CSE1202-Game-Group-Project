...
class player{
    ...
    void keyboard_movement(float screenW,float screenH){
        if(IsKeyDown(KEY_W) && pos.y>90){pos.y-=5;}
        if(IsKeyDown(KEY_S) && pos.y<screenH-90){pos.y+=5;}
        if(IsKeyDown(KEY_A) && pos.x>90){pos.x-=5;}
        if(IsKeyDown(KEY_D) && pos.x<screenW-90){pos.x+=5;}
    }
    ...
    void draw(Camera2D c){
        ...
        Rectangle dest={pos.x,pos.y,player.width*2.0f,player.height*2.0f};
        DrawTexturePro(player,src,dest,{player.width*1.0f,player.height*1.0f},rotation,WHITE);
    }
    void unload(){
